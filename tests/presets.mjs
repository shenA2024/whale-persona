...
const cfg = {
  ...
  persona: { enabled: true, userName: '徐石', selfNameFlash: '鲸鱼姐姐', character: '旧正文', legacyField: '保我' },
...
t('P5 没出现的字段保持原样:', next.persona.userName === '徐石' && next.persona.selfNameFlash === '鲸鱼姐姐')
...
// ── 预设不带记忆 ────────────────────────────────────────────────────────
const fromCfg = P.presetFromConfig({ thinkingLanguage: 'zh-CN', persona: { userName: '徐石', character: 'x' }, memory: { enabled: true, entries: [{ text: '私事', on: true }] } }, { id: 'snap', label: '快照' })
...
writeFileSync(process.env.DSH_WHALE_CONFIG, JSON.stringify({
  enabled: true, thinkingLanguage: 'en', persona: { userName: '徐石', character: '旧' },
  context: { memory: true }, forge: { enabled: true },
}, null, 2), 'utf8')
