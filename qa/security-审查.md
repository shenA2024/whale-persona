...
| 2026-09-26 | **sec-4（0.17.0 外部评审整改）** | v0.17.0（已发 npm） | 外部评审 2 个 P0 + 4 个 MED 全部处置；探针新增 **S16**（纪律文案 ↔ 引擎语义互钉，带自测夹具）；`prepublish-check` 新增 **P5** 契约漂移黄牌；`PASS true` + `SELFTEST true`（caught 含 S16）；**无阻塞项**（明细见 §5） |
| 2026-09-26 | **sec-5（示例文案匿名化）** | v0.17.0（已发 npm） | 示例 / 截图 / 测试 fixture 里的实际形象换成中性示例，两张文档截图按新文案重出；探针新增 **S17**（示例外貌三要素组合，带自测夹具）；`PASS true` + `SELFTEST true`（caught 含 S17）；**无阻塞项** |
