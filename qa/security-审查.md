...
| 关联项：示例文本与人设形象同值 | `examples/demo-config.json` 等处的通用示例原来写「年龄档 + 性别 + 身高」，且与人设形象**同值**（但没有任何独特标识：不含任何外观硬规格词） | **已按维护者决定换值**（2026-09-26）：统一改成中性通用示例「你是一位资深后端工程师」，`docs/images/` 两张截图重出；新增探针 **S17** 锁住这个组合模式（见 §4.4） |
