...
| 2026-09-26 | **sec-5（示例文案匿名化）** | v0.17.0（未发 npm） | 示例 / 截图 / 测试 fixture 里的实际形象换成中性示例，两张文档截图按新文案重出；探针新增 **S17**（示例外貌三要素组合，带自测夹具）；`PASS true` + `SELFTEST true`（caught 含 S17）；**无阻塞项** |
...
| 关联项：示例文本与人设形象同值 | `examples/demo-config.json` 等处的通用示例原来写「年龄档 + 性别 + 身高」，且与人设形象**同值**（但没有任何独特标识：没有鱼鳍耳、没有尾鳍、没有围裙小鲸鱼） | **已按维护者决定换值**（2026-09-26）：统一改成中性通用示例「你是一位资深后端工程师」，`docs/images/` 两张截图重出；新增探针 **S17** 锁住这个组合模式（见 §4.4） |
...
- 探针新增 **S17 SEC_SHAPE**（示例里不得出现「年龄 / 性别 + 身高」的可指认外貌组合）—— S14 的词表只认人名与关系，这类模式它看不见；判据只钉组合、不钉单词，避免误报成灾。
