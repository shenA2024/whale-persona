 ...
 * 分层选择（0.17.0 起）：tier=core 的条目**永远注入**、不参与上限竞争（maxEntries 只约束竞争池，core 不占名额）；
 * 未写 tier 的与 tier=hot 的一起按上面那套相关性参与；tier=cold 与落选的条目**不展开正文**，只进【记忆目录】索引，
...
function inboxDiscipline(cfg, active) {
  try {
    ...
    return '\n\n【长期记忆 · 入库纪律】\n'
      ...
      + '**层级 tier**：**缺省 = hot**（未写 tier 的条目参与项目相关性竞争，超额时正文不展开、只留【记忆目录】索引）——'
      + '缺席成本高的（安全边界、终局契约、指针类「去哪查」、关系口径）**必须显式写 "core"**，"core" = 永远注入；'
      + '"cold" = 从不展开正文，只进【记忆目录】索引。'
